"""Final accuracy test with original translator."""
import numpy as np
import torch
import sys
...
print("="*70)
print("ACCURACY TEST with Original Translator")
print("="*70)
...
for name, path, expected in tests:
    ...
    translated = translate_spectrogram(raw, a0, a1, a2)
    ...
    print(f"\n{name}: {hits}/{len(expected)} in top-5")
...
print(f"\n{'='*70}")
print(f"TOTAL: {total_hits}/{total_expected} ({100*total_hits/total_expected:.0f}%)")
print("="*70)

# Also test WITHOUT translation for comparison
print("\n\nWITHOUT TRANSLATION (baseline):")
print("-"*50)

total_hits_notrans = 0
for name, path, expected in tests:
    raw = np.genfromtxt(path, delimiter=',', skip_header=1, dtype=np.float32)
    probs = infer(raw)  # NO translation
    
    sorted_idx = np.argsort(probs)[::-1]
    top5_isotopes = set(isotope_names[i] for i in sorted_idx[:5])
    hits = len(set(expected) & top5_isotopes)
    total_hits_notrans += hits
    print(f"  {name}: {hits}/{len(expected)} in top-5")

print(f"\nWITHOUT Translation: {total_hits_notrans}/{total_expected}")
print(f"WITH Translation: {total_hits}/{total_expected}")
